 ...
 *
 * Reads project niches from PROJECT.md, ensures a qmd collection
 * exists per niche, sets context, and triggers index updates.
 *
 * Usage:
 *   bun run sync-qmd-collections.ts --project-slug shoshin
 *   bun run sync-qmd-collections.ts --project-slug shoshin --dry-run
 *   bun run sync-qmd-collections.ts --project-slug shoshin --skip-embed
 */
...
export function parseArgs(argv: string[] = process.argv.slice(2)): ParsedArgs {
  ...
  if (!result.success) {
    ...
    console.error(
      "Usage: bun run sync-qmd-collections.ts --project-slug <slug> [--dry-run] [--skip-embed]",
    );
    process.exit(1);
  }
...
async function regenerateQmdYml(
  ...
  const collectionsBlock = niches
    .map(({ slug, context }) => {
      const wikiPath = `companies/oopz/.evidence/wiki/${slug}`;
  ...
  const content = [
    "# Oopz qmd collections",
    "# One collection per niche. Collection name MUST match niche slug.",
    "# Paths are relative to the repo root (companies/oopz/).",
    "# DO NOT edit this file directly; use assets/scripts/sync-qmd-collections.ts",
    "# which reads projects/<slug>/PROJECT.md:niches and keeps this in sync.",
    "",
    "collections:",
    collectionsBlock,
    "",
...
export async function syncCollections(opts: {
  ...
  const { projectSlug, dryRun, skipEmbed } = opts;
  ...
  const niches = extractNiches(projectContent);
  result.nichesFound = niches;

  if (niches.length === 0) {
    console.log("No niches found in PROJECT.md");
    return result;
  }
...
async function main(): Promise<void> {
  const { projectSlug, dryRun, skipEmbed } = parseArgs();

  if (dryRun) {
    console.log("[dry-run] No qmd commands or file writes will occur.");
  }

  const result = await syncCollections({ projectSlug, dryRun, skipEmbed });
