...
No agent has write access to `skills/`. The write boundary table in `companies/shoshin/README.md` makes this explicit. BF skills are read-only from the agent graph's perspective.
...
1. Create the skill directory under `companies/shoshin/skills/<skill-name>/`.
...
If the new skill is a niche profile, follow the niche-profile naming convention: `<niche-slug>-niche-profile`. See `companies/shoshin/README.md` for the full "How to Add a Niche" walkthrough.
