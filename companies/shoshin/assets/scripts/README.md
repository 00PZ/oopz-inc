...
4. `bun run fetch-x-posts.ts --company-slug shoshin --niche-slug world-mobile`
5. `bun run fetch-web-article.ts --company-slug shoshin --niche-slug world-mobile`
6. `bun run kb-to-markdown.ts --company-slug shoshin --niche-slug world-mobile --source-type x-posts` (invoked automatically by fetch scripts, or run manually)
...
| Script | Args | Purpose |
|---|---|---|
| `fetch-x-posts.ts` | `--company-slug` `--niche-slug` | Fetch curated X/Twitter posts from tweet-curator-pg into project KB |
| `fetch-web-article.ts` | `--company-slug` `--niche-slug` | Fetch web articles via RSS/scrape into project KB |
| `kb-to-markdown.ts` | `--company-slug` `--niche-slug` `--source-type` | Project KB rows to markdown files in `.evidence/knowledge/` |
