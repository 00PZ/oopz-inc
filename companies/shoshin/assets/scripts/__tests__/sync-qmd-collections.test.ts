...
describe("extractNiches", () => {
  test("parses niches from COMPANY.md", () => {
    const content = loadFixture("COMPANY.md");
...
describe("parseArgs", () => {
  test("parses --company-slug", () => {
    const args = parseArgs(["--company-slug", "shoshin"]);
    expect(args.companySlug).toBe("shoshin");
    expect(args.dryRun).toBe(false);
    expect(args.skipEmbed).toBe(false);
  });

  test("sets dryRun true when --dry-run passed", () => {
    const args = parseArgs(["--company-slug", "shoshin", "--dry-run"]);
    expect(args.dryRun).toBe(true);
  });

  test("sets skipEmbed true when --skip-embed passed", () => {
    const args = parseArgs(["--company-slug", "shoshin", "--skip-embed"]);
...
describe("syncCollections", () => {
  ...
  beforeAll(async () => {
    ...
    const companyContent = loadFixture("COMPANY.md");
    const skillContent = loadFixture("test-niche-niche-profile/SKILL.md");

    await fsWriteFile(join(tempRoot, "COMPANY.md"), companyContent);
    await fsWriteFile(
