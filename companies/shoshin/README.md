...
A Paperclip agent company. Multi-niche short-form social media content engine. First niche: World Mobile.
...
**Two tiers**: Company owns agents/teams/skills. Niches are the knowledge boundary (one Postgres schema, one knowledge directory, one wiki per niche). Adding a niche = additive. Neither requires agent changes.
...
└────────┼─────────────────────────┼────────────────────────┼─────────────────┘
         ...
         ▼                         ▼                        ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│ SHOSHIN-KB (Postgres, per-company DB)                                       │
│                                                                             │
...
└──────────────────┬──────────────────────────────────────────────────────────┘
                   ...
                   ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│ qmd INDEX (~/.cache/qmd/shoshin/)  BM25 + vector + LLM re-ranking          │
...
- Source DBs (tweet-curator-pg) are read-only from Shoshin's perspective. One-way flow.
- `shoshin-kb` is Shoshin's copy, not a cache. It has its own shape.
...
```
Per-company DB, schema-per-niche isolation:
...
Future niches get their own schema within shoshin-kb.
Future sibling companies (separate oopz-inc packages) get their own DB.
```
...
# 3. Company: append to niches list
# COMPANY.md → niches: [world-mobile, defi-lending]

# 4. Adapters: run fetch scripts for new niche
bun assets/scripts/fetch-x-posts.ts --company-slug shoshin --niche-slug defi-lending
bun assets/scripts/fetch-web-article.ts --company-slug shoshin --niche-slug defi-lending
bun assets/scripts/kb-to-markdown.ts --company-slug shoshin --niche-slug defi-lending
...
# Run adapters
bun assets/scripts/fetch-x-posts.ts --company-slug shoshin --niche-slug world-mobile
bun assets/scripts/fetch-web-article.ts --company-slug shoshin --niche-slug world-mobile
bun assets/scripts/kb-to-markdown.ts --company-slug shoshin --niche-slug world-mobile
...
│   ├── intelligence-seed/        proposed skill diffs per niche (Friday 17:30)
│   ├── morning-brief/
│   ├── nightly-qmd-reindex/
...
│   ├── weekly-content-sprint/    Strategist, Monday 10:00
│   └── weekly-wiki-lint/
│
...
Shoshin ingest jobs (planned, stubs in assets/scripts/):
  shoshin-fetch-x-posts          every 30min    tweet-curator-pg → shoshin-kb
  shoshin-fetch-web-article      every 60min    web-scrape → shoshin-kb
  shoshin-to-markdown            after fetches  shoshin-kb → .evidence/knowledge/
  shoshin-fetch-clippings        daily 07:00    raw/clippings/ → shoshin-kb
```

Upstream scrapers own source DBs. Shoshin ingest jobs pull from source DBs into `shoshin-kb`. `kb-to-markdown.ts` projects to files. Agents read files. Paperclip Routines handle agent execution. Trigger.dev (once wired) handles data-pipeline scripts. The two systems never touch the same concern.
...
- Additional sibling brands (separate company packages in the oopz-inc portfolio)
...
- Obsidian per-niche vault integration
